...
export interface AuditPromptMessage {
  role: 'user' | 'assistant'
  content: { type: 'text'; text: string }
}

export interface AuditPromptResult {
  messages: AuditPromptMessage[]
  [key: string]: unknown
}
...
export interface AuditPromptDefinition {
  ...
  handler: (args: Record<string, unknown>) => AuditPromptResult
}

const userText = (text: string): AuditPromptResult => ({
...
export const AUDIT_PROMPTS: ReadonlyArray<AuditPromptDefinition> = [
  {
    name: 'full-dyor',
    ...
    handler: (args) => userText(
      `Run a full DYOR scan on this token.
...
- Whether I should buy, avoid, or monitor`
  ...
  },
  {
    ...
    handler: (args) => userText(
      `Quick safety check on this token before I buy.
...
End with overall risk level and whether it's safe to deploy.`
  ...
  },
  {
    ...
    handler: (args) => userText(
      `Scan this wallet for security risks.
...
- Tell me which approvals to revoke first at revoke.cash`
  ...
  },
  {
    ...
    handler: (args) => {
      ...
      return userText(
        `Verify the social presence of this project.
