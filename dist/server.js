...
export async function runServer() {
    ...
    for (const tool of TOOLS) {
        server.tool(tool.NAME, tool.DESCRIPTION, tool.inputShape, async (input) => {
            try {
    ...
    for (const prompt of AUDIT_PROMPTS) {
        ...
        server.prompt(prompt.name, prompt.description, shape, (args) => {
    ...
    for (const resource of AUDIT_RESOURCES) {
        server.resource(resource.name, resource.uri, { description: resource.description, mimeType: resource.mimeType }, resource.handler);
    }
